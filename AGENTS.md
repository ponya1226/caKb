...
- 自動保存品質の評価はADR 0016に従い、月別件数と理由コードだけを端末内に保存する。レシート内容、金額、支出ID、利用者識別子を集計処理へ渡さず、外部へ送らない。
...
- 管理者メニューでこの端末の自動登録率、要確認理由、Undo率、要確認時の総額修正率を確認・消去できる。
...
- ADR 0016の集計を外部送信する、またはレシート・支出・利用者を特定できる値を追加する。
