...
- 「毎度ありがとうございます」など一般的な来店・利用・購入へのお礼文を店舗候補から除外
- `支店名 00(0000)9876`、`支店名 TEL 00-0000-9876`形式では、電話番号だけを除去して支店名を候補として保持
- 直前のブランド名と正規化した支店名を組み合わせ、業態説明や冒頭文より優先
- 税区分前置型食品スーパーの匿名fixtureへ挨拶行を追加し、店舗名候補の単体回帰テストを追加
- 保存schemaや店舗カテゴリルール形式は変更していないためADR追加なし

検証結果:

- `npm.cmd run lint`: 成功
- `npm.cmd run test`: 33ファイル、165件成功
- `npm.cmd run test:receipt-quality`: 匿名コーパス18件、各精度指標100%、決済後数値混入0件、誤High 0件
...
- 実端末で同型レシートを再読取し、ブランド名と支店名が組み合わされた店舗名になることを確認する
- 店舗候補抽出を `receiptParser.ts` から独立モジュールへ分離する

## 2026-08-16 Tax-Prefixed Grocery Receipt Parsing

目的: 会計券の時刻を支払総額、括弧付き電話番号や精算機番号を品目として誤認せず、税区分と4桁商品コードが前置された食品スーパーの品目を店舗名に依存せず取得する。

主な変更:

- `お会計券 18:16`を支払総額ラベルとして扱わず、時刻トークンを金額候補から除外
- `00(0000)0000`形式の電話番号を品目候補から除外
- `外8 0012 商品名 ¥498`と`外10 0041 商品名 ¥5`形式を、買上点数との複合構造から既存の食品スーパーprofileで認識
- 構造profileでは最初の商品コード行まで品目解析を開始せず、精算機番号・担当者番号を除外。ただし直前の通貨記号付き金額単独行は、座標復元時の商品金額先行として保持
- 品目金額の下限だけを1円へ広げ、少額の袋代を品目として保持
- 同型の匿名fixtureを追加し、共有品質コーパスを18件へ拡張
- 保存schema、外部サービス、OCR Providerは変更していないためADR追加なし

検証結果:

- `npm.cmd run lint`: 成功
- `npm.cmd run test`: 33ファイル、164件成功
- `npm.cmd run test:receipt-quality`: 匿名コーパス18件、構造特徴10種、各精度指標100%、決済後数値混入0件、誤High 0件
- `npm.cmd run build`: 成功
- `npm.cmd run test:e2e`: mobile Chromium 12件成功
- `git diff --check`: 成功
...
- 実端末で同型レシートを再読取し、総額2,393円と7品目、品目合計2,217円が表示されることを確認する
- profile適用根拠の匿名診断表示と、品目行分類の独立モジュール化は継続課題
...
- 匿名fixtureへ、商品金額の同一行・改行、税集計、分割合計、支払、釣銭、電子マネー残高、数値フッター、列順崩れ、部分OCRの10構造特徴を追加
- fixtureごとに品目・総額候補へ混入してはいけない決済後数値を定義し、全体・レイアウト別・構造特徴別に混入件数を集計
- 分割合計、現金、釣銭、会員ランク数値を含む形式と、税込合計、カード決済、ポイント金額を含む形式の匿名fixtureを追加し、共有コーパスを15件へ拡張
- 明示的な支払総額を認識済みの場合は預り金から総額候補抽出を終了。支払方法の金額は重大な合計競合の検知と、合計未印字時のフォールバックのため維持
- 品目抽出は従来どおり最初の集計境界で終了し、決済・フッター後に再開しない
- Firestore、IndexedDB、支出、要確認Inbox、バックアップ、CSV、Google Sheetsの保存schemaは変更なし。ADR追加対象となる方針変更なし

検証結果:

- `npm.cmd run lint`: 成功
- `npm.cmd run test`: 29ファイル、150件成功
- `npm.cmd run test:receipt-quality`: 共有コーパス15件、10構造特徴、全精度指標100%、決済後数値混入0件、誤High 0件
- `npm.cmd run build`: 成功。Firebase import構成と約1,020KBのmain chunkに関する既知警告のみ
- `npm.cmd run test:e2e`: mobile Chromium 12件成功
- `git diff --check`: 成功
...
- 家族が実際に利用する未収録POS形式を、個人情報を除いた同一構造の匿名fixtureとして継続追加する
- fixture数だけでなく各構造特徴の最低件数を増やし、単一fixtureへの依存を減らす
- `receiptParser.ts`の構造分類、総額抽出、品目対応付けを独立モジュールへ段階分割する

## 2026-08-16 Receipt Structure Boundaries

目的: 店舗固有の除外語追加ではなく、一般的なレシートの印字構造に基づき、支払総額、支払方法、釣銭、残高、フッターより後の数値を商品として抽出しないようにする。

主な変更:

- レシート行を小計、税、支払総額、支払、決済後、フッターの構造境界へ分類する共通判定を追加
- `合`と`計`が別行になる場合や、`計 ¥300`のように後半行へ金額が付く場合も支払総額境界として認識
- 通常の品目抽出は最初の集計・支払境界で終了し、後続の会員ランク、ポイント、カード情報から再開しないよう変更
- 支払総額候補は釣銭、残高、または取引後フッターより前だけを評価し、後続の金額を大きさだけで選ばないよう変更
- 小計後へ移動した商品金額列の補完範囲を、最終支払額、支払、釣銭、残高、フッターより前へ限定
- 電子マネー残高fixtureへ取引後の会員ランク金額を追加し、分割合計とフッター除外、補完範囲を単体テストで検証
- Firestore、IndexedDB、支出、要確認Inbox、バックアップ、CSV、Google Sheetsの保存schemaは変更なし。ADR追加対象となる方針変更なし

検証結果:

- `npm run lint`: 成功
- `npm run test`: 29ファイル、148件成功
- `npm.cmd run test:receipt-quality`: 共有コーパス13件、全指標100%、誤High 0件
...
- 家族が利用するレシートで、支払総額やフッターの表現が未登録の形式を匿名fixtureへ継続追加する
- `receiptParser.ts`の構造境界、金額抽出、品目対応付けを独立モジュールへ段階分割する

## 2026-08-16 Home-center Split-price Candidate Selection

目的: 品目金額が別行や小計後へ移動し、単語座標から復元した行が一部欠落した場合でも、部門コードや会員情報を品目と誤認せず、完全な品目集合を選択する。

主な変更:

- `#0012 商品名`形式の先頭番号を品目金額ではなく部門コードとして除外し、次行の`#98`を品目金額として対応付け
- 小計、合計、現計などの集計開始位置を品目領域の終端とし、後続する会員ランクやポイント対象金額を品目候補へ混入させないよう変更
- Google Visionの単語座標側とOCR全文側からそれぞれ品目候補を作り、印字点数と小計への一致数、件数差、金額差を順に比較して、より整合する候補集合を採用
- 品目金額の次行化、後半3品目の金額列移動、部分的な座標復元、会員情報混入を同時に再現する匿名ホームセンターfixtureを追加
- Firestore、IndexedDB、支出、要確認Inbox、バックアップ、CSV、Google Sheetsの保存schemaは変更なし。ADR追加対象となる方針変更なし

検証結果:

- 共有コーパス13件: 総額一致率100%、品目完全一致率100%、品目適合率100%、品目再現率100%、誤High 0件、不要な要確認0件
- `npm run lint`: 成功
- `npm run test`: 29ファイル、146件成功
- `npm.cmd run test:receipt-quality`: 1ファイル、2件成功
- `npm run build`: 成功。Firebase import構成と約1,020KBのmain chunkに関する既知警告のみ
- `npm.cmd run test:e2e`: mobile Chromium 12件成功

残課題:

- 同じ実画像を家族端末から再読取し、総額6,027円と6品目、品目合計5,482円が表示されることを確認する
- レイアウト証拠の比較処理を今後のparser分割時に独立モジュールへ移し、候補を採用した根拠も診断レポートへ追加できる構造にする
...
- `計`単独行と`外税計`を区別し、ポイント対象額、会員ランク金額、預り金、釣銭を支払総額候補から除外
- Google Visionの単語座標から復元した行が全文と同じ支払総額を示す場合は、位置関係を保つ座標側の候補を採用
- 4桁の商品部門コードを品目価格として扱わず、商品名から除去
- 座標情報がない場合は、印字点数、未対応の商品名数、連続金額、その直後の小計がすべて一致するときだけ分離した金額列を補完し、曖昧対応として要確認へ送る
- 明細税額と税額合計が併記された場合は明細税額を優先し、10.0%などの税率を税額として加算しない
- ホームセンター形式を匿名共有コーパスへ追加して12件へ拡張し、判定ルールを `receipt-confidence-v5` へ更新
- Firestore、IndexedDB、支出、要確認Inbox、バックアップ、CSV、Google Sheetsの保存schemaは変更なし。ADR追加対象となる方針変更なし

検証結果:

- 共有コーパス12件: 総額一致率100%、品目完全一致率100%、品目適合率100%、品目再現率100%、誤High 0件、不要な要確認0件
- `npm run lint`: 成功
- `npm run test`: 28ファイル、142件成功
...
- 同じ実画像を家族端末から再読取し、支払総額6,027円と6品目が表示されることを確認する
- 座標なし補完は安全のため要確認に残す。実利用で頻発する場合はProxyから単語座標が欠落していないかを先に調査する
- `receiptParser.ts` の正規化、領域分類、行対応、整合性判定を段階分割し、レイアウトプロファイルへ移行する
...
- 割引率行を商品名待ち行列から分離し、商品名の後に割引額と商品価格が続く並びをそれぞれ独立した明細として対応付け
- `特`だけが金額の前に残る行を金額専用行として扱い、連続する商品名へ順番に対応付け
- 同じ商品名・同じ金額の行を重複排除せず、実際の複数購入として保持
- 支出再編集時は品目明細を初期展開し、長い明細のスクロール対応を継続
- 実店舗情報を含まない17明細、税抜5,195円、税415円、総額5,610円の回帰fixtureを追加

検証結果:

- `npm run lint`: 成功
- `npm run test`: 27ファイル、131件成功
...
- 同一行のOCR重複も保持するため、過剰抽出時は品目合計不一致として要確認になる。実レシート結果を匿名fixtureへ追加して調整する
...
- ADR 0016を追加し、自動登録品質を端末内の月別集計だけで評価する方針を決定
- 自動保存、要確認、複数枚確認、10秒Undo、要確認保存時の総額変更、破棄をhousehold別に集計
- 店舗名、日付、金額、品目、画像、OCR全文、支出ID、UID、メールアドレスを集計対象外として型とAPIの境界を限定
- 最新12か月の集計だけを `localStorage` に保持し、Firestore、バックアップ、CSV、Google Sheets、外部ログへ送信しない構成を維持
- 通常画面へ操作を追加せず、折りたたみの管理者メニューに今月の自動登録率、要確認理由、Undo率、要確認時の総額修正率と消去操作を追加
- 管理者・家族のスマホ実機でHigh自動保存、Low要確認、Undo、Inbox復元・保存・破棄が動作することを確認済みとしてRoadmapへ反映

検証結果:

- `npm run lint`: 成功
- `npm run test`: 26ファイル、123件成功
- `npm run build`: 成功。既存の大きいchunk警告のみ
- `npm run test:e2e`: mobile Chromium 8件成功
...
- 集計は端末単位で、家族の別端末とは合算されない
- 総額修正率は要確認保存時だけを対象とし、自動保存後に支出一覧で行った編集は追跡しない
- 1か月程度の利用結果と匿名fixtureを確認するまでConfidence閾値は変更しない
- 複数枚は引き続き `batch_flow` 理由の要確認で、画像単位自動保存は次フェーズ
