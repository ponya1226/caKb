...
- コンビニ、スーパー、専門店、食品スーパー、ホームセンター、部分失敗の17件を共有する匿名レシート品質コーパスと、総額・品目・Confidence判定の定量ゲート
...
- 2桁部門コードと明示小計点数から選ぶ食品スーパーPOS profile。店舗名に依存せず、非該当形式はgeneric解析へfallback
...
- 共有品質コーパスは17件で、家族が実際に利用するPOS形式の種類とOCR崩れをまだ十分には網羅していない。
