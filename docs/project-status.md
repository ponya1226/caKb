...
- 利用画面の用語を一般利用者向けに統一。OCR、Google Vision、Tesseract.js、Firestore、IndexedDB、JSON、Proxyなどの実装用語を、レシート読み取り、オンライン読み取り、端末内読み取り、クラウド、この端末、バックアップへ置換
...
- 複数品目でOCRの読み取り順が大きく入れ替わる場合、文字列だけでの品目・金額対応付けには限界がある。必要に応じてGoogle Visionの座標情報を候補抽出へ渡す設計が必要。
