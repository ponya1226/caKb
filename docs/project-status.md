...
- `receipt-confidence-v3` で、品目合計が総額または印字税額加算後の総額と完全一致しない読み取りに加え、小計差分補完や複数候補の曖昧対応を要確認へ送るゲート
...
- 品目抽出は匿名回帰fixtureで検証しているが、品目完全一致率、適合率、再現率、誤High件数を継続計測する品質レポートは未実装。
...
- `receipt-confidence-v3` の小計差分補完・曖昧対応件数を家族端末で確認し、誤Highがないことと要確認率を評価する
- `receiptParser.ts` の正規化、領域分類、行対応、整合性判定の境界を定義し、最初のPOS形式プロファイルを食品スーパー形式で作る
- 利用頻度の高いPOS形式ごとに匿名の正解fixture群を作り、品目完全一致率、適合率、再現率、誤High件数を出すテストレポートを追加する
...
- `receipt-confidence-v3` の品目不一致、小計差分補完、曖昧対応の理由件数を確認し、不要な要確認がないか実レシートで評価する
