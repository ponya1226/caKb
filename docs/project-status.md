...
- コンビニ、スーパー、専門店、食品スーパー、ホームセンター、部分失敗の16件を共有する匿名レシート品質コーパスと、総額・品目・Confidence判定の定量ゲート
...
- `receiptParser.ts` から正規化と構造分類は分離したが、品目名と金額の対応付け、POS形式別プロファイルはまだ集中している。
- 共有品質コーパスは16件で、家族が実際に利用するPOS形式の種類とOCR崩れをまだ十分には網羅していない。
...
- `receiptParser.ts` の品目行対応と整合性判定を分離し、最初のPOS形式プロファイルを食品スーパー形式で作る
