...
- レシートをヘッダー、品目、集計、支払総額、決済、フッターへ分類し、集計開始後に通常の品目抽出を再開しない共通構造境界
- 明示的な支払総額を認識した後は預り金から金額候補抽出を終了し、釣銭、残高、取引後フッター以降の数値を除外。支払方法の金額は合計競合の検知に維持し、小計後の列崩れ補完は最終支払額より前だけに制限
- `計`単独行と`外税計`を区別し、預り金、釣銭、残高、ポイント対象額、会員ランク金額を支払総額候補から除外
- 明細税額と税額合計が同時に印字された場合は明細税額だけを照合へ利用し、税率の数値を税額に含めない
- コンビニ、スーパー、専門店、食品スーパー、ホームセンター、部分失敗の19件を共有する匿名レシート品質コーパスと、総額・店舗名・品目・Confidence判定の定量ゲート
- 商品金額の同一行・改行、税集計、分割合計、支払、釣銭、電子マネー残高、数値フッター、列順崩れ、部分OCRの10構造特徴別評価と、決済後数値混入0件の品質ゲート
- 分割合計と電子マネー残高を各2fixture以上とする最低件数ゲート
- OCR文字正規化を `receiptText.ts`、小計からフッターまでの構造分類を `receiptStructure.ts` へ分離し、候補抽出とは独立して単体テスト
- 2桁部門コード、または`外8/外10 + 4桁商品コード`と明示点数から選ぶ食品スーパーPOS profile。店舗名に依存せず、非該当形式はgeneric解析へfallback
- `03*`のように税率記号が2桁部門コード直後へ付く商品行でも、部門コードを金額候補から除外して次行の商品価格と対応
- 会計券に印字された時刻を総額候補から除外し、括弧付き電話番号、商品欄前の精算機番号・担当者番号を品目へ混入させない構造境界
- 1円以上の品目金額を許容し、税区分前置の商品行に含まれる少額の袋代を保持
- レシート冒頭の来店お礼文を店舗候補から除外し、電話番号付き支店行から支店名を保持して直前のブランド名と結合
- 店舗候補の除外、正規化、ブランド・支店結合、順位付けを `receiptShop.ts` へ分離し、8件の匿名fixtureで店名一致率を品質ゲート化
- 品目名正規化と商品・数量・税・決済・住所・コード・割引・集計ラベルの行分類を `receiptLineItemClassification.ts` へ分離し、分類規則を独立単体テスト化
- 列順補完と小計差分補完を `receiptLineItemReconciliation.ts` へ分離し、印字点数、小計、割引、残差範囲が競合する場合は補完しない不変条件を独立単体テスト化
- 総額・品目で共有する数値トークン解析を `receiptAmounts.ts` へ分離し、OCR数字補正、時刻・税率・商品コード・数量の除外、少額商品、割引符号を独立単体テスト化
- 印字点数、小計、小計後の金額列を `receiptLineItemEvidence.ts` で生成し、整合性補完へ渡す証拠を候補抽出から分離
- 品目名と金額の待機・対応を `receiptLineItemAssociation.ts`、OCR全文候補と座標候補の整合性選択を `receiptLineItemSelection.ts` へ分離
...
- 店舗候補抽出は独立モジュール化したが、一般的な印字構造による限定的なヒューリスティックであり、実績のない店舗形式を個別ルールで推測しない。
...
- `receiptParser.ts` は座標行復元、日付、総額候補順位付け、品目解析の合成を引き続き担当する。新しい実利用上の失敗根拠がない責務分割は行わず、匿名fixtureで変更価値を確認してから分離する。
- POS profileは食品スーパー1形式だけで、家族が利用する他形式にはgeneric解析を使用している。
- 共有品質コーパスは19件で、家族が実際に利用するPOS形式の種類とOCR崩れをまだ十分には網羅していない。
...
- 家族端末で食品スーパーprofileの要確認率と品目順を確認し、profile適用根拠を診断用コピーへ安全に追加する
- 追加のparser分割より家族端末の実績収集を優先し、具体的な誤認が確認された場合は数値トークン・証拠生成の直接テストと匿名fixtureを先に追加する
- 家族の利用頻度が高い新しいPOS形式やOCR崩れを匿名共有コーパスへ追加し、十分な実績がある形式だけ次のprofile候補にする
- 新しい店舗ヘッダー構造で誤候補が見つかった場合は、実店舗名を使わない期待店名付きfixtureへ追加し、店名一致率を維持する
