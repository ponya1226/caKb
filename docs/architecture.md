...
通常の品目候補は品目領域からだけ抽出し、小計、税、支払総額、決済のいずれかへ入った後に抽出を再開しません。釣銭、残高または取引後フッター以降の金額は支払総額のフォールバック候補にも含めません。例外として、Google Visionの読み順で商品金額列だけが小計後へ移動した場合は、小計から最終支払額までの範囲に限り、印字点数、未対応品目数、連続金額、復元小計がすべて一致するときだけ補完し、要確認へ送ります。最終支払額、支払方法、釣銭、残高、フッターより後の金額をこの補完に利用しません。
