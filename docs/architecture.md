...
`receiptAmounts.ts` は、総額と品目で共有する金額トークンの正規化、OCR数字補正、時刻・税率・商品コード・数量の除外、割引符号、品目名との分離を担当します。`receiptLineItemEvidence.ts` は、印字点数、小計、小計後に分離した金額列を同じ金額解析から生成します。`receiptLineItemReconciliation.ts` は、抽出済み品目、未対応の商品名とこの証拠を受け取り、列順補完と小計差分補完の可否を純粋関数で判定します。点数または算術整合性が不足する場合は補完せず、店舗・OCR行分類・Reactには依存しません。
