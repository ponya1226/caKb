...
候補抽出では「合計」「税込」「現計」「お買上計」などの周辺にある金額を優先します。競合する強い金額、別額の残高、日付・店舗・カテゴリの未解決、取得済み品目と総額・税額の不一致があれば要確認にし、安全に解決できた場合だけ自動保存します。Provider選択、範囲調整、画像補正は通常画面へ表示しません。
