...
- `receipt-confidence-v3` では、取得済み品目の合計が総額、または印字税額を加えた総額と完全一致しない場合を要確認にする。
...
- 新しい修正は同じレイアウト群の匿名fixtureを追加し、単一レシートだけでなく既存fixture全体で評価する。
- 品目完全一致率、品目適合率・再現率、要確認率、誤High件数を計測し、誤High件数0をリリース条件とする。
