...
- 家族の利用頻度が高いPOS形式から、支店名に依存しないレイアウトプロファイルとして切り出す。最初の対象として、2桁部門コードと明示小計点数を持つ食品スーパーprofileを追加した。
- 品目名と金額の待機・対応を `receiptLineItemAssociation.ts`、OCR全文候補と座標候補の整合性選択を `receiptLineItemSelection.ts` へ分離した。
- コンビニ、スーパー、専門店、食品スーパー、ホームセンター、部分失敗の17件を共有匿名コーパスへまとめ、総額、品目、Confidence判定を同じfixtureから評価する品質ゲートを追加した。
